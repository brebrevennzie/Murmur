...
export function getInitialPrograms(): SyllabusProgram[] {
  ...
  localStorage.setItem('tutor_syllabus_programs', JSON.stringify(DEFAULT_PROGRAMS));
  return DEFAULT_PROGRAMS;
}

export function savePrograms(programs: SyllabusProgram[]) {
  localStorage.setItem('tutor_syllabus_programs', JSON.stringify(programs));
}
...
export function getInitialStudents(): Student[] {
  const data = localStorage.getItem('tutor_students_db');
  if (data) {
    try {
      return JSON.parse(data);
    } catch (e) {
      console.error('Failed to parse from localStorage, using initial state', e);
      return INITIAL_STUDENTS;
    }
  }
  // Store default
  localStorage.setItem('tutor_students_db', JSON.stringify(INITIAL_STUDENTS));
  return INITIAL_STUDENTS;
}

export function saveStudents(students: Student[]) {
  localStorage.setItem('tutor_students_db', JSON.stringify(students));
}
